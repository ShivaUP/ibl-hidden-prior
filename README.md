...
Train **only on synthetic** IBL-like sessions; **primary** ranking on held-out synth; **secondary** frozen transfer to real mouse behavior (no mouse fine-tune). Neural VE / MOs–vlOFC claims are **parked**.
...
| Path | What it answers | Regenerated by |
|---|---|---|
| `reports/v2/figures/by_model/{model}/multipanel_diagnostics.png` | Kyan/Shrijana 2×2: train CE; psychometric by block P(right); switch zero-evidence (−20…+30, 0.2↔0.8); example session true prior vs zero-evidence preference | `08` then `10_make_figures.py` |
| `reports/v2/figures/comparison/synth_heldout_accuracy.png` | Synth choice accuracy ranking | same |
| `reports/v2/figures/comparison/synth_heldout_history_gap.png` | Zero-evidence history gap | same |
| `reports/v2/figures/comparison/real_transfer_accuracy.png` | Transfer: correct-side vs mouse-choice | same |
| `reports/v2/figures/figure_catalog.json` | Machine-readable catalog | same |
| `docs/TICK_AND_CHANNELS.md` | Empirical tick schedule + channel coding | (docs; from `04_fit`) |
