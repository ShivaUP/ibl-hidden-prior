...
This project studies how mice update a hidden prior in the International Brain Laboratory decision task and whether that computation is better explained by recurrent dynamics or explicit online inference. The main model comparison is between a standard task-trained RNN, a predictive-coding RNN, and a Bayesian model, with emphasis on behavior, belief updating around block switches, and neural alignment in prior-related regions.

**v1 results:** see [`reports/v1_report.md`](reports/v1_report.md).  
**Data is not in git** (raw/processed/checkpoints ignored): see [`docs/DATA.md`](docs/DATA.md).
...
Rebuild datasets and models from manifests + scripts (downloads via public openalyx). Full recipe: [`docs/DATA.md`](docs/DATA.md).
...
In prior-related regions such as MOs and vlOFC, which model best matches neural population dynamics around block switches, and does that advantage survive after behavior matching?
...
- VISp or nearby early visual areas
- MOs and/or MOp
- orbvl / vlOFC for expectation-related and prior-related signals

### Planned comparisons

- Neural decoding of prior-related signals
- Trial-by-trial latent alignment between model and brain
- Representational dynamics around block switches
- Behavior-matched neural comparison across models
